...
def instructor_list(request):
    ...
    return render(request, 'booking/instructor_list.html',
                  {'instructors': instructors})


@login_required
def book_lesson(request, lesson_id=None):
    """Requires users to be logged in, allows the user to submit
    booking information, saves the booking to the database, &
    displays a success message upon a successful booking."""
    lesson = None
    if lesson_id:
...
def register(request):
    """Saves user information to the database, & provides feedback
    to the user upon successful registration, also redirects to
    the homepage."""
    if request.method == 'POST':
...
def user_login(request):
    """Authenticating users, & provides feedback to the user upon successful
    or unsuccessful login attempts, redirects to home page on success."""
    if request.method == 'POST':
...
def user_logout(request):
    """Handles user logout by ending the user's session and
...
def delete_instructor_profile(request):
    """Deletes the instructor profile. Ensures that only
    authenticated users who are registered instructors can
    delete their profiles. Provides feedback to the user upon
    successful deletion & redirects to the home page. A user
    who is not an instructor tries to access page, they are
